...
import { ArrowLeft, ArrowRight, Save, BookOpen, Plus, Rocket } from 'lucide-react';
...
const STEPS = [
  ...
  { label: 'Videos', description: 'Sube el contenido' },
];
...
export function CourseCreator() {
  ...
  const { courseId, setCourseId, modules, addModule, removeModule, updateLesson, reset, loadFromCourse } = useInstructorStore();
  ...
  useEffect(() => {
    ...
    api.get(`/courses/${editId}`)
      ...
      .catch((err) => {
        if (!cancelled) {
          setErrorMsg(err?.response?.data?.error || 'No se pudo cargar el curso para editar');
        }
      })
      .finally(() => {
        if (!cancelled) {
  ...
  const handleSaveModules = async () => {
    ...
    try {
      for (let mi = 0; mi < modules.length; mi++) {
        ...
        let moduleId = mod.id;
        if (!moduleId) {
          const mr = await api.post(`/courses/${courseId}/modules`, { title: mod.title });
          moduleId = mr.data.id;
  ...
  const handleVideoUploaded = async (moduleIndex: number, lessonIndex: number, url: string) => {
    ...
    try {
      ...
      console.error('Failed to attach video to lesson', err);
    }
  };
    ...
    <div className="max-w-4xl mx-auto space-y-4 sm:space-y-6 pb-12 px-0">
        ...
        <div className="bg-card border border-border rounded-xl p-4 sm:p-6">
          ...
          <form onSubmit={handleSubmit(handleSaveBasicInfo)} className="space-y-4 sm:space-y-6">
            ...
            <div className="flex justify-end pt-2">
              ...
              >
                {saving ? <span className="animate-spin w-4 h-4 border-2 border-white border-t-transparent rounded-full" /> : <ArrowRight className="w-4 h-4" />}
      ...
      {step === 2 && (
        <div className="space-y-4">
          <div className="bg-card border border-border rounded-xl p-4 sm:p-6">
            <h2 className="text-lg sm:text-xl font-bold mb-1 flex items-center gap-2">
              <Rocket className="w-5 h-5 text-primary" /> Subir Videos
            </h2>
            <p className="text-sm text-muted-foreground mb-5">Sube el video para cada lección. Puedes publicar sin completarlos todos.</p>

            <div className="space-y-6">
              {modules.map((module, mi) => (
                <div key={mi} className="space-y-3">
                  <h3 className="font-semibold text-sm text-muted-foreground uppercase tracking-wide border-b border-border pb-2">
          ...
          <div className="flex flex-col-reverse sm:flex-row justify-between gap-3">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              >
                {publishing ? <span className="animate-spin w-4 h-4 border-2 border-white border-t-transparent rounded-full" /> : <Rocket className="w-4 h-4" />}
